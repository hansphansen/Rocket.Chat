...
const assetFolder = '../../../node_modules/joypixel-assets';
const emojiJsonFile = `${assetFolder}/emoji.json`;

if (!fs.existsSync(emojiJsonFile)) {
	console.error(`${emojiJsonFile} doesn't exist.`);
	console.error("Maybe you need to run 'meteor npm install joypixel-assets' or 'meteor npm install'?");
} else {
 ...
 */\n\n`;
	...
	for (let category in emojisByCategory) {
		...
		const spritePath = `../../../public/packages/joypixel/${category}-sprites.png`;
		...
		nsg(
			{
				...
				stylesheetOptions: {
					...
					category: category,
					spritePath: `/packages/joypixel/${category}-sprites.png`,
					pixelRatio: 1,
				},
			},
			function (err) {
...
`;
	fs.writeFileSync('../client/joypixel-sprites.css', spriteCss, {
		encoding: 'utf8',
		flag: 'w',
	});
}
