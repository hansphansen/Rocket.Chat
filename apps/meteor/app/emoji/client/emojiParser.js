...
const emojiParser = (message) => {
	...
	const emojis = Array.from(checkEmojiOnly.querySelectorAll('.emoji:not(:empty), .joypixel:not(:empty)'));

	let hasText = false;

	if (!isIE11) {
		const filter = (node) => {
			if (node.nodeType === Node.ELEMENT_NODE && (node.classList.contains('joypixel') || node.classList.contains('emoji'))) {
